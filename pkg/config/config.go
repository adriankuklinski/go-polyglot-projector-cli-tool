...
func getArgs(opts *Opts) ([]string, error) {
	...
	if operation == Add {
		if len(opts.Args) != 3 {
			return nil, fmt.Errorf("add requires 2 arguments, but recieved %v", len(opts.Args)-1)
		}
	...
	if operation == Remove {
		if len(opts.Args) != 2 {
			return nil, fmt.Errorf("remove requires 1 arguments, but recieved %v", len(opts.Args)-1)
		}
	...
	if len(opts.Args) > 1 {
		return nil, fmt.Errorf("print requires 1 or 0 arguments, but recieved %v", len(opts.Args))
	}
